...
export const getPendingTestimonials = async (): Promise<
  PendingTestimonial[]
> => {
  try {
    const pendingTestimonialsJson = localStorage.getItem(
...
export const addPendingTestimonial = async (
  ...
  try {
    ...
    const updatedPendingTestimonials = [
      ...pendingTestimonials,
      newPendingTestimonial,
    ];
    localStorage.setItem(
      PENDING_TESTIMONIALS_KEY,
      JSON.stringify(updatedPendingTestimonials)
    );
...
export const approveTestimonial = async (id: string): Promise<void> => {
  try {
    ...
    const updatedPendingTestimonials = pendingTestimonials.filter(
      (t) => t.id !== id
    );
    localStorage.setItem(
      PENDING_TESTIMONIALS_KEY,
